...
{
    public partial class RootOtherForm : UserControl
    {
        public RootOtherForm()
        {
            InitializeComponent();
            //定义数据库实体
            ...
            //获取报修表中所有数据
            var records = database2Entities2.Transfe;
            foreach (var record in records)
            {
                //新建一行记录，显示上去
                var oneLine = new OneMaintain(record.RoomId, record.Reason);
        ...
        public void RemoveOneRecond(object sender, ref OneMaintain oneLine)
        {
            ...
            //新建一条记录
            Transfe oneRecond = new Transfe();
            //填入对应的房号
            oneRecond.RoomId = oneLine._roomId;
            //提交删除语句
            database2Entities2.Entry<Transfe>(oneRecond).State = EntityState.Deleted;
            //保存设置
